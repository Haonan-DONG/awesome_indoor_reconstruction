...
class ScanNetDataset(Dataset):
    ...
    def build_list(self):
        with open(
            os.path.join(
                self.datapath, self.tsdf_file, "fragments_{}.pkl".format(self.mode)
            ),
            "rb",
        ) as f:
            metas = pickle.load(f)
    ...
    def read_cam_file(self, filepath, vid):
        intrinsics = np.loadtxt(
            os.path.join(filepath, "intrinsic", "intrinsic_color.txt"), delimiter=" "
        )[:3, :3]
        intrinsics = intrinsics.astype(np.float32)
        extrinsics = np.loadtxt(
            os.path.join(filepath, "pose", "{}.txt".format(str(vid)))
        )
    ...
    def read_scene_volumes(self, data_path, scene):
        if scene not in self.tsdf_cashe.keys():
            ...
            for l in range(self.n_scales + 1):
                ...
                full_tsdf = np.load(
                    os.path.join(data_path, scene, "full_tsdf_layer{}.npz".format(l)),
                    allow_pickle=True,
                )
