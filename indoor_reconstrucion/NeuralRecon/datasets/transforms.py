...
class ResizeImage(object):
    ...
    def __init__(self, size, intrinsic_resize=True):
        self.size = size
        self.intrinsic_resize = intrinsic_resize
...
class RandomTransformSpace(object):
    ...
    def __call__(self, data):
        ...
        if (not self.random_rotation) and (not self.random_translation):
            ...
            voxel_dim_old = (
                torch.tensor(data["tsdf_list_full"][0].shape) * self.voxel_size
            )
            ...
            start = torch.Tensor([xmin, ymin, zmin]) - self.padding_start
                ...
                -torch.Tensor(voxel_dim) * self.voxel_size
                + torch.Tensor([xmax, ymax, zmax])
                + self.padding_end
            )
        ...
        data["vol_origin"] = torch.tensor(
            self.origin, dtype=torch.float, device=T.device
        )
    ...
    def transform(self, data, transform=None, old_origin=None, align_corners=False):
        ...
        if "tsdf_list_full" in data.keys():
            ...
            for l, tsdf_s in enumerate(data["tsdf_list_full"]):
                ...
                occ_vol[
                    (tsdf_vol < 0.999) & (tsdf_vol > -0.999) & (weight_vol > 1)
                ] = True
                ...
                coords_world_s = (
                    2 * coords_world_s / (torch.Tensor(old_voxel_dim) - 1).view(3, 1)
                    - 1
                )
