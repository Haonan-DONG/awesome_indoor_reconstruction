...
from tools.process_huawei_data import process_data


parser = argparse.ArgumentParser(
    description="NeuralRecon Real-time Demo on Huawei dataset."
)
parser.add_argument(
...
test_dataset = ARKitDataset(
    cfg.TEST.PATH, "test", transforms, cfg.TEST.N_VIEWS, len(cfg.MODEL.THRESHOLDS) - 1
)
...
with torch.no_grad():
    for frag_idx, sample in enumerate(tqdm(data_loader)):
        ...
        #               3. VIS_INCREMENTAL
        save_scene = (
            (cfg.SAVE_SCENE_MESH and frag_idx == frag_len - 1)
            or cfg.SAVE_INCREMENTAL
            or cfg.VIS_INCREMENTAL
        )
