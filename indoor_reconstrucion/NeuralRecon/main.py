...
from tools.pose_visialization.camera_pose_visualizer import CameraPoseVisualizer
...
visualizer = CameraPoseVisualizer([-2.5, 10], [-8, 5], [0, 10])


def args():
    parser = argparse.ArgumentParser(
        description="A PyTorch Implementation of NeuralRecon"
    )
...
optimizer = torch.optim.Adam(
    model.parameters(), lr=cfg.TRAIN.LR, betas=(0.9, 0.999), weight_decay=cfg.TRAIN.WD
)
...
def train():
    ...
    if cfg.RESUME:
        ...
        if len(saved_models) != 0:
            ...
            optimizer.param_groups[0]["lr"] = state_dict["optimizer"]["param_groups"][
                0
            ]["lr"]
        ...
        optimizer.param_groups[0]["lr"] = state_dict["optimizer"]["param_groups"][0][
            "lr"
        ]
        start_epoch = state_dict["epoch"] + 1
